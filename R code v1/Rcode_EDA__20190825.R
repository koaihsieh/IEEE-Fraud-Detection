...
####  In the subset of data with identity, what is the difference between Fraud/Non-Fraud groups ???
two_group_hist <- function(cfield, N=20){
## cfield <- "id_10"
  par(mfrow=c(3,1))
  data0 <- subset(train_identity_with_isFraud)[,c("isFraud",cfield)]
  data1 <- subset(data0,isFraud==1)

  h0 <- hist(data0[[cfield]], plot=F, n=N)
  h1 <- hist(data1[[cfield]], plot=F, breaks=h0$breaks)
  plot(h1, xlab=cfield, main=paste("Is Fraud Group  (",round(sum(is.na(data1[[cfield]]))/nrow(data1)*100,2),"% NA)",sep=""))
  plot(h0, xlab=cfield, main=paste("All Data  (",round(sum(is.na(data0[[cfield]]))/nrow(data0)*100,2),"% NA)",sep=""))
  num <- h1$counts
  den <- h0$counts;  den[den==0] <- 1
  fraudrate <- round(num/den*100,2)
  cols <- (fraudrate>quantile(fraudrate,.69))+1
  cols[fraudrate==max(fraudrate)] <- 6
  xlimit <- range(h0$mids)+c(-1,1)*(max(h0$mids)-min(h0$mids))/(10+length(h0$counts))
  ylimit <- c(0,max(fraudrate*1.2))
  plot(h1$mids, fraudrate, type='l', xlab=cfield, ylab="Fraud rate", xlim=xlimit, ylim=ylimit)
  points(h1$mids, fraudrate, xlab=cfield, ylab="Fraud rate", xlim=xlimit, ylim=ylimit, col=cols, pch=16)
  points(h1$mids[cols>1], fraudrate[cols>1], xlab=cfield, ylab="Fraud rate", xlim=xlimit, ylim=ylimit, col=cols[cols>1], cex=3)
  texts <- round(h0$counts/sum(h0$counts)*100,3)
  cols[texts>10] <- 4
  text(h1$mids, fraudrate+max(fraudrate)/6, paste(texts,"%",sep=""), col=cols)
  text(h1$mids[cols>1], (fraudrate-max(fraudrate)/8)[cols>1], paste("FR = ",fraudrate[cols>1],"%",sep=""), col=cols[cols>1])
  title(main="Fraud rate in each bin (% of data in each bin & top 30% fraud rates in bins colored red)")
  mtext(paste(cfield,"column"), side = 3, line = -1.5, outer = TRUE)
}
two_group_hist("id_01")
two_group_hist("id_02")
two_group_hist("id_03")
two_group_hist("id_04")
two_group_hist("id_05")
two_group_hist("id_06")
two_group_hist("id_07")
two_group_hist("id_08")
two_group_hist("id_09")
two_group_hist("id_10")
two_group_hist("id_11")
two_group_hist("id_13")
two_group_hist("id_14")
two_group_hist("id_17")
two_group_hist("id_18")
two_group_hist("id_19")
two_group_hist("id_20")
two_group_hist("id_21")
two_group_hist("id_22")
two_group_hist("id_24")
two_group_hist("id_25")
two_group_hist("id_26")
two_group_hist("id_32")
